        ...
        if (!dreamInfo) {
          ...
          if (createDream) {
            const userLikeDream = await User_like_dream.create({
              // dream 레코드 추가했다면 userlikedrea 조인레코드 추가
              user_id: userId,
            ...
            if (userLikeDream) {
              return res
                .status(200)
                .json({ message: "like this dream", likeId: createDream.id });
            }
          }
        } else {
          const hasJoinId = await User_like_dream.findOne({
            where: { user_id: userId, dream_id: dreamInfo.id },
          });
          if (!hasJoinId) {
            const userLikeDream = await User_like_dream.create({
              user_id: userId,
            ...
            if (userLikeDream) {
              return res
                .status(200)
                .json({ message: "like this dream", likeId: dreamInfo.id });
            }
          } else {
            return res
              .status(200)
              .json({ message: "already liked it", likeId: dreamInfo.id });
          }
        }
      }
    }
