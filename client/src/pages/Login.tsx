...
function Login(): JSX.Element {
  ...
  const handleSubmit = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    axios
      ...
      .then((res) => {
        res.data.profile =
          typeof res.data.profile !== 'object' &&
          typeof res.data.profile === 'string'
            ? res.data.profile
            : 'data:image/png;base64, ' +
              Buffer.from(res.data.profile, 'binary').toString('base64')
  ...
  const naverLogin = async () => {
    ...
    loginId.getLoginStatus(function (status: any) {
      if (status) {
        const email = loginId.user.email
  ...
  return (
    <Container>
      <LogInBox>
        ...
        <HaveAccount>
          <p>회원이 아니신가요?</p>
          <LinkedP to="/signup">회원가입</LinkedP>
        </HaveAccount>
      </LogInBox>
    </Container>
  )
